...
export default function Page() {
	...
	const [selectedChatId, setSelectedChatId] = useState<Id<"chats"> | null>(
		searchParams.get("id") as Id<"chats"> | null
	);
	...
	const handleChatSelect = (id: Id<"chats">) => {
	...
	return (
		<>
			...
			<div className="grid grid-rows-12 h-full w-60 min-w-60 border-r-[1px]">
				...
				<input
					className="row-span-1 outline-none border-b-[1px] h-full w-full px-2"
				...
				<div className="row-span-10 overflow-auto no-scrollbar">
					{chats &&
						chats.map((chat) => {
							if (
								chat.status.toLowerCase() === status.toLowerCase() &&
								chat.createdBy.toLowerCase().includes(searchValue.toLowerCase())
							) {
								return (
									<Item
										...
										description={removeHTMLTags(
											chat.messages[chat.messages.length - 1]?.format.includes("image")
												? "Image"
			...
			{selectedChatId === "" || selectedChatId === null ? (
				<div className="flex grow items-center justify-center">Select a chat to continue</div>
			) : (
				<ConversationSection id={selectedChatId} />
			)}
		</>
	);
}
...
function Item(props: {
	...
	return (
		<div
			...
			className={`relative px-3 py-5 min-h-20 border-b-[1px] hover:cursor-pointer hover:bg-slate-100 transition-all text-gray-500 hover:text-black ${
				searchParams.get("id") === props.id ? "bg-slate-100 text-black " : ""
			} text-gray-500`}
		>
			...
			{props.isNew && <div className="absolute top-2 right-2 h-2 w-2 bg-accent rounded-full" />}
		</div>
	);
}
...
function ConversationSection(props: {
	...
	const { user } = useUser();
	const chat = useQuery(api.chats.get.byId, { id: props.id });

	const [isRoomInformationOpen, setIsRoomInformationOpen] = useState(false);

	if (!props.id || Array.isArray(chat)) {
		return (
	...
	return (
		<div className="grid grid-rows-12 h-full w-full">
			<h1 className="flex items-center justify-between row-span-1 text-lg font-semibold border-b-[1px] px-6">
				{chat == null ? (
					<div className="h-4 w-4/12 skeleton" />
				) : (
					<>
						{trunc(chat.createdBy, 40)}
						<InformationCircleIcon
							className="size-7 hover:cursor-point"
							onClick={() => setIsRoomInformationOpen(!isRoomInformationOpen)}
						/>
					</>
				)}
			</h1>
