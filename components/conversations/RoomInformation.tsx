...
export default function RoomInformation(props: RoomInformationProps) {
	...
	const renderContact = () => {
		...
		return <RoomInformationItem title="Contact">
	...
	const renderTags = () => {
		if(!Array.isArray(roomInformation.tags)) return;
	...
	return <section className={`overflow-y-scroll overflow-x-hidden bg-white border-l-[1px] h-full absolute top-0 right-0 transition-all no-scrollbar ${props.isOpen ? "w-96" : "w-0"}`}>
