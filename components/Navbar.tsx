...
export default function Navbar(props  : {
	...
	return (
		<>
			<nav className="relative navbar bg-neutral justify-between px-4">
				...
				<div className="space-x-2 hidden lg:block">
					<Link href="#" className="btn btn-accent"> Transcript </Link>
					{
						props.conversation?.status === CONVERSATIONSTATUS.CLOSED ? <Link href="#" className="btn btn-success" onClick={props.onNewChat}>New Chat</Link> : <Link href="#" className="btn btn-error" onClick={props.onEndChat}>End Chat</Link>
					}
				</div>
			</nav>

			{isOpen && (
				<div className="fixed flex justify-end items-center p-2 top-16 w-full min-h-fit gap-2 z-50 ">
					<button className="btn btn-accent grow">
						Transcript
					</button>
					{
						props.conversation?.status === CONVERSATIONSTATUS.CLOSED ? 
